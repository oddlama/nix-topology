...
let
  ...
  allNodeInterfacePairs = concatMap (
    node: map (interface: { inherit node interface; }) (allInterfacesOf node)
  ) allNodes;

  # Helper to add a value to a list if it doesn't already hold it
  addIfNotExists = list: x: if elem x list then list else list ++ [ x ];

  extractNetworkDefinitions =
    nodeId: node:
  ...
  nodeInterfaceNetworkReplacementType = types.attrsOf (
    types.submodule {
  ...
  effectiveNodeDefinitions =
    (mergeDefinitions options.nodes.loc nodeInterfaceNetworkReplacementType
      options.nodes.definitionsWithLocations
    ).mergedValue;
  ...
  networkDefiningInterfaces = foldl' recursiveUpdate { } (
    flatten (mapAttrsToList extractNetworkDefinitions effectiveNodeDefinitions)
  );

  # A list of all connections between all interfaces. Bidirectional.
  connectionList = unique (
    flatten (
      flip mapAttrsToList config.nodes (
        nodeId: node:
  ...
  propagateNetworks =
    state: snode: sinterface: dnode: dinterface:
  ...
  propagateToConnections =
    state: src:
    #builtins.trace "propagate via connections of ${src.node}.${src.interface}"
    (
      # Fold over each connection of the current interface
      foldl' (acc: dst: propagateNetworks acc src.node src.interface dst.node dst.interface) state (
        connections.${src.node}.${src.interface} or [ ]
      )
    );
  ...
  propagateLocally =
    ...
    (
      ...
      foldl' (
        acc: dstInterface:
        if
          src.interface != dstInterface
          && any (f: f dstInterface) config.nodes.${src.node}.interfaces.${src.interface}.sharesNetworkWith
        then
          propagateNetworks acc src.node src.interface src.node dstInterface
        else
          acc
  ...
  propagatedNetworks =
      ...
      interfacesWithMultipleNets = flatten (
        flip mapAttrsToList converged (
          ...
          flip mapAttrsToList ifs (
            interface: nets: optional (length (attrNames nets) > 1) { inherit node interface nets; }
          )
        )
      );
    in
    warnIf (interfacesWithMultipleNets != [ ]) ''
      ...
      ${concatLines (
        flip map interfacesWithMultipleNets (
          x:
          " - ${x.node}.${x.interface}:\n"
          + concatLines (
              ...
              types.submodule (submod: {
                options = {
                  ...
                  type = mkOption {
                    ...
                    default =
                      if
                        hasPrefix "wlp" submod.config.id
                        || hasPrefix "wlan" submod.config.id
                        || hasPrefix "wifi" submod.config.id
                      then
                        "wifi"
                      else
                        "ethernet";
                    ...
                    // optionalAttrs config.topology.isMainModule {
                      default =
                        let
                          ...
                          sharedNetwork = if sharedNetworks == { } then null else builtins.head (attrNames sharedNetworks);
                        in
                ...
                config = {
                  ...
                  sharesNetworkWith = mkIf (
                    config.topology.isMainModule && nodeSubmod.config.deviceType == "switch"
      ...
      flip map (attrValues config.nodes) (
        ...
        flip map (attrValues node.interfaces) (
          ...
          [
            ...
            (config.lib.assertions.iconValid interface.icon "nodes.${node.id}.interfaces.${interface.id}")
          ]
          ++ flip map interface.physicalConnections (physicalConnection: {
            assertion =
              config.nodes ? ${physicalConnection.node}
              && config.nodes.${physicalConnection.node}.interfaces ? ${physicalConnection.interface};
      ...
      flip map (attrValues config.nodes) (
        ...
        flip map (attrValues node.interfaces) (
          ...
          flip map interface.physicalConnections (
            ...
            let
              otherNetwork =
                config.nodes.${physicalConnection.node}.interfaces.${physicalConnection.interface}.network or null;
            in
