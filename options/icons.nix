...
f {
  ...
  config = {
    ...
    lib.assertions.iconValid = icon: path: {
      assertion =
        icon != null -> (!hasPrefix "/" icon -> hasAttrByPath (splitString "." icon) config.icons);
    ...
    lib.icons.get =
      ...
      if icon == null then
        null
      # Icon is a path
      else if hasPrefix "/" icon then
        icon
      # Icon is a valid icon name
      else if hasAttrByPath attrPath config.icons then
        (getAttrFromPath attrPath config.icons).file
      # Icon is not valid
      else
        throw "Reference to unknown icon ${icon} detected. Aborting.";
