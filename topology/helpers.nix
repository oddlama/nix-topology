...
rec {
  ...
  mkSwitch =
    ...
    mkMerge [
      ...
      ])
      {
        ...
        interfaces = mkMerge (
          flip mapAttrsToList connections (
            interface: conns: { ${interface}.physicalConnections = toList conns; }
          )
          ++ flip concatMap interfaceGroups (
            group:
            flip map group (interface: {
  ...
  mkRouter = name: args: mkSwitch name (args // { deviceType = "router"; });

  mkDevice = name: args: mkSwitch name (args // { deviceType = "device"; });
}
