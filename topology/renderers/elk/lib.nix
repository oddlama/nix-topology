...
in
rec {
  mapAttrsRecursiveInner =
    f: set:
    let
      recurse =
        path: set:
        f path (
          flip mapAttrs set (name: value: if isAttrs value then recurse (path ++ [ name ]) value else value)
        );
    in
    recurse [ ] set;
  ...
  elkifySchema =
    ...
    flip mapAttrsRecursiveInner schema (
      path: value: if path != [ ] && elem (last path) attrsToListify then listify path value else value
    );

  mkEdge =
    from: to: reverse: extra:
    if reverse then
      mkEdge to from false extra
    else
      {
        edges."${from}__${to}" = {
          sources = [ from ];
  ...
  pathStyleFromNetworkStyle =
    style:
    {
      ...
      // optionalAttrs (style.secondaryColor != null) { background = style.secondaryColor; };
      dotted = {
        stroke = style.primaryColor;
  ...
  interfaceLabels =
    ...
    // optionalAttrs (interface.mac != null) { "50-mac" = mkLabel interface.mac 1 netStyle; }
