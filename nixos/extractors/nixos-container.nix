    ...
    type = types.attrsOf (
      types.submodule (submod: {
        options._nix_topology_config = mkOption {
          ...
          internal = true;
          default = if submod.options.config.isDefined then submod.config.config else null;
          description = ''
            ...
            interfaces = mkMerge (
              flip map container.macvlans (
                ...
                let
                  ...
                  iface_container =
                    if lib.length splitString > 1 then lib.elemAt splitString 1 else "mv-" + iface_host;
                in
                {
                  ${iface_container} = {
                    type = "macvlan";
