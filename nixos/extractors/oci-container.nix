{
  ...
  containers =
    mapAttrsToList (name: cfg: cfg // {_name = name;})
  ...
  matchesById = mapAttrs (_: spec: let
    oci = spec.oci or null;
  in
    if oci == null
    then []
    else filter (c: matchesRepos c.image oci.repos) containers)
  serviceRegistry;

  mkServiceFromHits = id: spec: hits:
    mkIf (hits != []) (let
      firstHit = head hits;
  ...
  traefikHostInfoFn = cfg: let
    labels = cfg.labels or {};
    ruleKeys =
      filter (k: hasSuffix ".rule" k && strings.hasPrefix "traefik.http.routers." k)
      (attrNames labels);
    hosts = concatStringsSep " " (map (k: let
  ...
  generated = listToAttrs (mapAttrsToList
    (id: spec: {
      name = id + "-oci";
...
in {
  options.topology.extractors.oci-container = {
    enable = mkEnableOption "OCI container extractor" // {default = true;};
